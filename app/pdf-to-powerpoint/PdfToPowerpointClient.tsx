...
import { Presentation, File as FileIcon, Loader2, Download, AlertCircle, RefreshCw, CheckCircle2 } from "lucide-react";
import { useDropzone } from "react-dropzone";
import { motion, AnimatePresence } from "framer-motion";
import { queuePdfToPowerpointJob } from "@/app/actions/queuePdfToPowerpointJob";
...
export default function PdfToPowerpointClient() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const res = await queuePdfToPowerpointJob(originalStoragePath);
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-center mb-10">
        <div className="bg-red-100 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5 border border-red-200">
        ...
        <p className="text-gray-500 text-lg max-w-xl mx-auto leading-relaxed">
          Convert your PDF documents to editable PowerPoint (PPTX) presentations.
        </p>
      </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {file && status !== "completed" && status !== "error" && (
                <motion.div
                  ...
                  className="w-full"
                >
                  <div className="bg-red-50 border border-red-100 rounded-2xl p-6 mb-8 flex items-center gap-4">
                  ...
                  <div className="flex justify-center gap-4 mt-8">
                    ...
                    <button 
                      ...
                      disabled={status === "processing"}
                      className="px-8 py-3 font-bold text-white bg-red-600 hover:bg-red-700 rounded-xl transition-all shadow-md shadow-red-200 hover:shadow-lg disabled:opacity-50 flex items-center gap-2"
                    >
                      {status === "processing" ? (
                        <><Loader2 className="w-5 h-5 animate-spin" /> Processing</>
                      ) : (
                        <><Presentation className="w-5 h-5" /> Convert to PPTX</>
                      )}
                    </button>
                  </div>
                </motion.div>
              ...
              {status === "completed" && downloadUrl && (
                ...
                >
                  ...
                  <p className="text-gray-500 mb-8 max-w-sm mx-auto">
                    Your PDF has been successfully converted to a PowerPoint presentation.
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4">
                    <a 
                      ...
                      className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-8 rounded-xl transition-all shadow-md shadow-red-200 hover:shadow-lg"
                    >
