...
export async function queueCompressJob(originalFilePath: string, totalSizeBytes: number) {
  try {
    ...
    const compressQueue = new Queue('pdf-heavy-jobs', { connection });
    
    await compressQueue.add('compress-pdf', {
      jobId,
      userId: user.id,
      originalFilePath
    });
    ...
    console.error("Failed to queue compress job:", error);
