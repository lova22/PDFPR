...
export async function queuePdfToWordJob(originalFilePath: string, totalSizeBytes: number) {
  try {
    ...
    const { data: jobRow, error: insertError } = await supabase
      ...
      .insert({
        user_id: user.id,
        job_type: 'pdf_to_word', // Matches DB check constraint
        status: 'pending',
        file_count: 1,
    ...
    await workerQueue.add('pdf-to-word', {
      jobId,
      userId: user.id,
      originalFilePath
    });
    ...
    console.error("Failed to queue PDF to Word job:", error);
